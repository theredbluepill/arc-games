...
2. **Task notebook** – [Create new task](https://www.kaggle.com/benchmarks/tasks/new), attach the dataset, then paste cells from `benchmarks/kaggle/arc_kaggle_notebook_template.py` (one `@kbench.task` per published task), or run `python3 benchmarks/kaggle/rebuild_kaggle_notebooks.py` to generate `benchmarks/kaggle/notebooks/*.ipynb`. For **every** stem under `environment_files/`, run `uv run python benchmarks/kaggle/export_kaggle_notebooks_all_stems.py` → `benchmarks/kaggle/notebooks/all/*.ipynb` (optional `notebook_export_overrides.json` for `max_steps` / `grid_size`). See `benchmarks/kaggle/notebooks/README.md` for **3.11 vs 3.12** / bootstrap deps.
